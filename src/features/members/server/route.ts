...
const app = new Hono()
    ...
    .delete("/:memberId", sessionMiddleware, async (c) => {
        ...
        if (allMembersInWorkspace.total === 1) return c.json({error: "Cannot delete the only member."});
    ...
    .patch("/:memberId", sessionMiddleware, zValidator("json", z.object({role: z.nativeEnum(MemberRole)})), async (c) => {
        ...
        if (allMembersInWorkspace.total === 1) return c.json({error: "Cannot downgrade the only member."});
